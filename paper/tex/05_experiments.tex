...
We evaluate the PEG numerically across four experiments, each isolating one
distinct question raised by the framework. \cref{tab:experiments-overview}
gives a one-line preview of each before we develop them in turn.

\begin{table}[t]
\centering
\caption{Overview of the numerical experiments.}
...
C5 & Functional form of the externality & 2-group, vary $\eps_O$ and proportion at fixed $\eps_T$ & $A_t$ monotone in $\eps_O$, crossover at $\eps_O{=}\eps_T$ (\cref{fig:C5}) \\
C6 & Grid-refinement of the NE locus & C1 at $K=44$ (double resolution) & NE locus is a smooth 1-D curve (\cref{fig:C6}) \\
C7 & Sensitivity to $B/C$ ratio (symmetric) & $B/C \in [0.25, 8]$, $K=22$ & Asymmetric NEs in $B/C \in [0.5, 4]$; collapse to diagonal at extremes (\cref{fig:C7}) \\
C8 & Asymmetric privacy weights & $C_1/C_2 \in [0.25, 4]$, $K=22$ & NE locus migrates: more-sensitive player takes the low-$\eps$ role (\cref{fig:C8}) \\
\bottomrule
\end{tabular}
\end{table}
...
question is whether they sample an analytical curve in
$(\eps_1, \eps_2)$-space or whether the grid resolution itself is generating
their multiplicity. The implicit first-order condition~\eqref{eq:foc}
defines two BR loci---one per player---whose intersection \emph{is} the NE
set; under symmetric players the two loci are reflections across the
diagonal. There is no closed form for either curve because $A_i$ has none,
but the curve is a one-dimensional locus and its shape is testable by grid
refinement.

We rerun C1 at $K = 44$ on the same $[\eps^{\min},\eps^{\max}]$ interval,
doubling the resolution along each axis. \cref{fig:C6} reports the
resulting BR loci with the pure NEs marked. The two curves
trace smooth, monotone-decreasing loci in the lower-left quadrant; their
intersections (the pure NEs) lie on a curved 1-D set off the diagonal. At
$K = 44$ we recover the same qualitative structure as at $K = 22$, but the
NEs become denser along the curve---each $K = 22$ NE has neighbours at
$K = 44$ that fall on the same curve. The asymmetric-pure-NE structure is
therefore not a grid artefact; it is the discrete sampling of a smooth
1-D locus in the action plane.
...
with NEs marked (right) at the refined resolution $K = 44$. The NE locus
in $(\eps_1, \eps_2)$ space is a smooth 1-D curve off the diagonal; doubled
grid resolution refines but does not change its shape.}
\label{fig:C6}
\end{figure*}
